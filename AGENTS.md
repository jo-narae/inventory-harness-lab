...
| **검증 규칙** — 무엇을 통과해야 끝인가 · 무엇이 판정하나 | `docs/harness/02-verification.md` **(원본)** | 범위 §0 · 두 층 §1 · 공통 기계 검증 §2 |
| **구현·검증 루프** — 반복 단위 · 최대 횟수 · 사람 개입 시점 | `docs/harness/03-loop.md` **(원본)** | 범위 §0 · 반복 단위 §1 · 흐름 §2 · 최대 횟수 §3 · `NEEDS_HUMAN` 시점 §4 |
