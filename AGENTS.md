...
| **검증 규칙** — 무엇을 통과해야 끝인가 · 무엇이 판정하나 | `docs/harness/02-verification.md` **(원본)** | 범위 §0 · 세 층 §1 · 공통 기계 검증 §2 · 게이트와 경고 §3 · Issue 단위 검증 §4 · LLM 검토 §5 · 사람 검증 §6 |
...
### 이슈 작업 요청이 들어오면 — 스킬을 쓸지 먼저 묻는다

`이슈 처리해줘` · `15번 이슈 해줘` 처럼 Issue 를 실제로 작업하라는 요청을 받으면,
구현에 들어가기 전에 **`issue-loop` 스킬을 쓸지 사람에게 먼저 묻는다.**

| 답 | 진행 |
|---|---|
| 쓴다 | `.claude/skills/issue-loop/SKILL.md` 로 들어가 그 절차대로 끝까지 돈다 — 브랜치 · 구현 · 검증 · Issue 코멘트 · 커밋 · 푸시 · PR · CI 대기 · LLM 검토 |
| 쓰지 않는다 | 스킬 없이 이 파일과 `docs/harness/` 의 규칙대로 수동으로 진행한다. 커밋 · 푸시 · PR 은 §3 대로 따로 요청받았을 때만 한다 |

묻는 이유는 두 경로의 **권한이 다르기** 때문이다. 스킬 경로는 그 호출을 커밋 · 푸시 · PR
생성의 요청으로 본다 — `03 §6.1` 이 사람의 요청을 요구하는 자리를 맨 앞에서 한 번에 당겨
받는 것이다. 수동 경로는 그 요청을 그때그때 따로 받는다. 어느 쪽을 원하는지는 사람만 답할
수 있고, 잘못 고르면 되돌리기 어려운 쪽은 스킬 경로다.

이 질문은 **한 번만** 한다. 답을 받은 뒤에는 그 경로의 규칙을 따르고 같은 작업 안에서 다시
묻지 않는다. 스킬 경로를 골랐다면 그 뒤로 멈추는 자리는 이슈 번호 확인과 `NEEDS_HUMAN`
둘뿐이다.
