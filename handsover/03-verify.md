...
## 9. 검증 원본 작성 — `docs/harness/02-verification.md`

`§3 남은 작업` 의 2번이다. **검증 규칙을 새로 발명하지 않고, 지금 실제로 돌아가는 하네스를 읽어
그대로 원본으로 굳혔다.** 이 문서 `§1`·`§7`·`§8` 과 `04-ci.md`·`05-issue-form.md` 가 그대로 입력이 됐다.

### 무엇을 읽고 썼나

| 입력 | 문서에 들어간 것 |
|---|---|
| `package.json` 의 `verify` 체인 | 8단계 파이프라인 표와 순서의 근거 |
| `scripts/verify/check-protected.ts` | 보호 경로 도출·비교 기준·승인 형식 (`§3.1`) |
| `scripts/reset-verify-db.ts` | 검증 DB 격리 (`§3.2`) |
| `scripts/check-architecture.ts` | A1~A4 와 범위·예외 3건 (`§3.3`) |
| `vitest.config.ts` · `tests/` | 테스트 소유 구분, `fileParallelism: false` 의 이유 (`§3.4`) |
| `verify-m1.ts` 의 `ok()` · `drift()` | 게이트와 경고의 구분 (`§4.1`) |
| `.github/ISSUE_TEMPLATE/maintenance.yml` | 종료 조건 ↔ 테스트 1:1, 전용 테스트 경로 (`§5`) |
| `.github/workflows/verify.yml` | CI 가 같은 명령을 그대로 돌린다 (`§2`) |

### 구조

| 절 | 정하는 것 |
|---|---|
| 0 | 범위 — 정하는 것 / 정하지 않는 것 + **검증 항목의 출처 표** |
| 1 | 검증의 두 층 — 기계(확정) · 사람(미정) |
| 2 | 공통 기계 검증 `npm run verify` — 8단계 표 · 순서의 근거 · CI 동일 실행 |
| 3 | 단계별 규칙 — Protect · Prepare · Arch · Test · Domain |
| 4 | 판정 규칙 — 게이트/경고 · **검사 불능은 실패** · 결과 보고 |
| 5 | Issue 단위 검증 — 1:1 대응 · 경로 · 경계 |
| 6 | 멈추는 지점 — 최대 횟수 · 보호 경로 · `NEEDS_HUMAN` |
| 7 | 반복 실행은 `03-loop.md` 몫이라고 명시하고 넘긴다 |
| 8 | 사람 검증 — ⬜ 미정 |
| 9 | 미정 목록 |

### 핵심 — 검증 원본도 사본을 만들지 않는다

`02-ssot-harness.md §6` 함정 1번("SSOT 문서에 요약·목록을 담으면 그게 또 사본이 된다")이
검증 문서에도 그대로 걸린다. 불변식 7개(`06 §9`)나 완료 기준 12개(`01 §7`)를 옮겨 적으면,
원본을 고쳐도 검증 문서가 옛 목록을 계속 주장한다.

그래서 `§0` 에 **항목 목록이 아니라 출처 표**를 뒀다. 무엇을 검증하는지는 원본이 말하고,
이 문서는 그것을 **언제 어떻게 판정하는지**만 말한다.

### 함께 반영한 것 — `00-ssot.md` 4곳

원본이 생겼으므로 등록부를 갱신했다 (`00-ssot.md §0.5`).

| 절 | 변경 |
|---|---|
| §1 | 검증 규칙 상태 `추후 생성` → `확정` |
| §2 | `### 검증 규칙` 절 신설 — 다루지 않는 것(항목 내용 · 반복 절차)까지 명시 |
| §3 | 검증 규칙 행 삭제. `구현·검증 루프` 한 줄만 남았다 |
| §5 | `Issue별 테스트` 대상 `추후 정할 …` → `tests/issues/` |

**`§5` 갱신으로 `§8` 이 남긴 3번 항목이 닫혔다.** `05-issue-form.md §5` 의 1번도 같은 항목이다.
`tests/issues/` 는 AI 가 작성·수정하는 영역이므로 보호 경로에는 들어가지 않는다 — 경로가 정해졌어도
`check-protected.ts` 의 판정은 그대로다.

### 라우터도 함께 움직였다 — `AGENTS.md` 3곳

`02-ssot-harness.md §6` 함정 5번("규칙을 SSOT에만 적으면 에이전트가 따르지 않는다")의 반대 방향이다.
원본이 생기면 라우터가 **틀린 말을 하기 시작한다.**

| 위치 | 변경 |
|---|---|
| 라우팅 표 검증 규칙 행 | `⬜ 원본 미작성 … 세 곳을 다 본다` → `docs/harness/02-verification.md` **(원본)** |
| 표 아래 문장 | `위 세 줄만 원본이다` → `(원본) 으로 표시한 행만` — 원본 행이 4개가 되어 **위치 기준이 깨졌다** |
| §3 작업 규칙 | `검증 원본이 없으므로 …` → 원본대로 돌리고 돌린 결과만 보고, 원본이 안 다루는 것만 이슈에 먼저 적는다 |

두 번째가 놓치기 쉽다. **행을 하나 더한 것만으로 "위 세 줄" 이라는 표현이 거짓이 된다.**
표에 행을 더할 때는 표를 가리키는 문장도 같이 본다.

### 검증 결과

| 확인한 것 | 결과 |
|---|---|
| `ALLOW_PROTECTED=… npm run verify` | **전 단계 통과** — Arch 57개 파일 위반 0 · Test 19/19 · Domain 불변식 전부 통과 · Build 15 routes |
| 승인 없이 `npm run verify:protected` | **exit 1** — `docs/harness/02-verification.md` 를 정확히 잡는다 |
| SSOT·AGENTS 반영 후 재실행 | 보호 경로 3건(`AGENTS.md` · `00-ssot.md` · `02-verification.md`)을 잡는다 |

문서만 바뀌었고 코드는 건드리지 않았다. 마크다운은 어느 단계도 검사하지 않는다.

### 주의

**문서를 쓰면서 자기 자신이 걸린다.** `docs/harness/` 가 보호 경로이므로 검증 원본을 작성·수정하는
커밋마다 승인이 필요하다. `§8` 의 "스스로를 잡는다" 와 같은 구조다.

```
Approved-Protected: docs/harness/00-ssot.md docs/harness/02-verification.md AGENTS.md
```

**초안 280줄을 사람이 다듬는 중이다.** 기록 시점에 파일은 51줄이고 `§2` 의 코드펜스가 열린 채 끝난다.
다듬는 방향은 수치 빼기(`불변식 7개` → `불변식`) · 문장 쪼개기 · 조사 앞 공백 제거다.
**`§9` 미정 3번(`§5` 와 어긋남)은 위에서 해소됐는데, 그 절이 아직 파일에 없어 지우지 못했다.**

### `§3 남은 작업` 현황

| # | 항목 | 상태 |
|---|---|---|
| 1 | CI 에서 동일한 `npm run verify` 실행 | 완료 (`§6` · `05-issue-form.md §1`) |
| 2 | `docs/harness/02-verification.md` 작성 | **이번에 완료** — 다듬는 중 |
| 3 | README 에 `npm run verify` 추가 여부 | 미정 (`02 §9` 4번) |
| 4 | 단독 `npm test` 도 검증 DB 로 격리할지 | 미정 (`02 §9` 2번) |
| 5 | 수동 검증(S1~S12 · 반응형 · 엣지)의 처리 방법 | 미정 (`02 §8` · `§9` 1번) |

3·4·5 는 사라진 게 아니라 **`02-verification.md §9` 미정 목록으로 자리를 옮겼다.**
원본이 생겼으므로 미결도 원본이 들고 있는 것이 맞다.

### 다음 시작점

1. **`02-verification.md` 다듬기를 마친다** — `§9` 미정 3번을 지운다 (`00-ssot.md §5` 갱신으로 해소됨)
2. **`docs/harness/03-loop.md`** — 마지막 미작성 원본. 이슈 하나를 받아 구현·검증·종료까지의 반복 절차.
   `02 §6`(멈추는 지점)과 Issue Form 6번 칸(최대 횟수)이 초안 입력이다
3. **첫 Issue 를 폼으로 열어 `tests/issues/` 의 첫 파일을 만든다** — 경로 규약이 실제로 쓸 만한지는
   한 번 채워봐야 안다 (`05-issue-form.md §5` 4번)
4. **`02 §9` 의 미정 7건 중 5·6·7번** — Arch 검사 범위·예외 확정, `A3` 인자명 `tx` 통일,
   스키마·설정 스냅샷 검사(`Movement @@index` · WAL). 7번은 코드가 `06` 과 어긋난 상태다
